"""
Задача 2 — Async: параллельный парсинг веб-страниц
====================================================
Использует aiohttp для асинхронных HTTP-запросов и asyncio.gather
для одновременного парсинга всех URL в одном потоке.

Это лучший подход для I/O-bound задач: event loop переключается
между корутинами пока те ждут ответа от сервера, не блокируя программу.
Требует меньше ресурсов чем threading/multiprocessing (нет накладных
расходов на создание потоков/процессов).

Примечание: psycopg2 синхронный — используем его в run_in_executor
чтобы не блокировать event loop при записи в БД.
"""
