"""
Задача 2 — Threading: параллельный парсинг веб-страниц
========================================================
Загружает HTML нескольких URL параллельно через потоки,
извлекает <title> и сохраняет в БД из ЛР1.

Подходит для I/O-bound задач: пока один поток ждёт ответа от сервера,
GIL освобождается и другие потоки могут работать.
Это главное отличие от CPU-bound задач — здесь threading эффективен.
"""
