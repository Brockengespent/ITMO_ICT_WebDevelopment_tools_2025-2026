...
============================
Вычисление суммы чисел от 1 до N с использованием процессов.

Особенность: каждый процесс имеет свой интерпретатор Python и свою память,
что полностью обходит ограничение GIL. Для CPU-bound задач это даёт
реальное ускорение, пропорциональное числу ядер процессора.

Недостаток: создание процессов дороже создания потоков — есть накладные расходы
на запуск и межпроцессное взаимодействие (IPC).
"""
